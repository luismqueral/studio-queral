...
# Always use current directory for deployment (cache is pre-built and committed)
# For local development, we can build cache from parent directory content
if os.environ.get('VERCEL') or os.environ.get('AWS_LAMBDA_FUNCTION_NAME'):
    # In deployment: use pre-built cache in content/ directory
    vault_path = current_dir
    cache_dir = "content/.cache"
else:
    # Local development: can build cache from actual content
    vault_path = os.path.join(current_dir, "..")
    cache_dir = ".cache"

# Initialize cache manager
cache_manager = PostCacheManager(vault_path, cache_dir)
